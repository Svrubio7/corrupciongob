  ...
  <div class="social-share-container my-4">
    <div class="flex items-center gap-2">
      ...
      <button 
        ...
        class="social-btn bg-palette-black text-white hover:bg-gray-800 transition-colors"
      ...
      <!-- Primary Social Media Buttons (Always Visible) -->
      <button 
        @click="shareOnFacebook"
        class="social-btn border border-gray-300 hover:bg-gray-50 transition-colors"
      ...
      <transition name="slide-fade">
        <div v-if="isExpanded" class="flex items-center gap-2">
          ...
          <button 
            ...
            :class="{ 'bg-green-50 border-green-300': copied }">
...
export default {
  ...
  computed: {
    ...
    shareText() {
      return `${this.title} ${this.shareUrl} a través de ${this.accountName}`
    },
    // Text without URL embedded (URL passed separately for platforms that handle it differently)
    shareTextWithAttribution() {
      return `${this.title} a través de ${this.accountName}`
    }
  },
  methods: {
    toggleExpanded() {
    ...
    shareOnLinkedIn() {
      // LinkedIn - title and URL separated for better preview
    ...
    shareOnTelegram() {
      ...
      const text = encodeURIComponent(this.shareTextWithAttribution)
      const url = `https://t.me/share/url?url=${encodeURIComponent(this.shareUrl)}&text=${text}`
      window.open(url, '_blank')
    },
    shareByEmail() {
      // Email with formatted subject and body
      const subject = encodeURIComponent(`${this.title} - ${this.accountName}`)
      const body = encodeURIComponent(`${this.title}\n\n${this.shareUrl}\n\na través de ${this.accountName}`)
